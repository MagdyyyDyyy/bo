...
async def start_search(update: Update, context):
    ...
    if not results:
        await update.message.reply_text("🤷🏻‍♂️ شكل الفيلم دا مش عندي ")
        return
...
async def handle_movie_selection(update: Update, context):
    ...
    if not final_link:
        await query.edit_message_text("تعذر استخراج رابط التحميل.")
        return
...
async def start_command(update: Update, context):
    await update.message.reply_text("أهلا بيك في بوت افلاميا .. ابعت اسم الفيلم 🧞‍♂️")
